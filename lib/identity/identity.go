...
// GetResetPasswordURL ...
func (id *IDP) GetResetPasswordURL(ctx context.Context, email, contURL string, origin string) (string, error) {
	actionCode := auth.ActionCodeSettings{
		URL: contURL,
	}
	verifyLink := ""
	...
	zicopReplace := strings.Replace(verifyLink, "https://zicops-one.firebaseapp.com/__/auth/action?", origin+"/reset-password?", 1)
